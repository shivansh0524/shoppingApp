...
<br>
 ...
 <div class="d-flex justify-content-center  mt-2 p-2" >
    ...
    <?php 
    $ss=$_SESSION['id'];
$sql="SELECT * FROM buyerdetails WHERE Serial_no='$ss'";
$result=mysqli_query($conn,$sql);
if(mysqli_num_rows($result)>0){
    while($row=mysqli_fetch_assoc($result)){
        ...
        $sale=0;
        $sql3="SELECT * FROM orders WHERE Delivery_Status='Delivered' AND Email='$Email'";
...
<p><span style="color:rgb(6, 94, 145)">BUYER</span></P>

<p><span>Gender:</span> &nbsp;<?php echo $Gender?> </p>
<p ><span>Email: &nbsp;</span><?php echo $Email?> </p>
<p ><span>Phone: &nbsp;</span><?php echo $Phone_no?> </p>
<p ><span>Address: &nbsp;</span><?php echo $Address?> </p>
<p ><span>State: &nbsp;</span><?php echo $State?> </p>
<p ><span>Total Purchase: &nbsp;</span> ₹<?php echo $sale?>.00 </p>

<p ><span>Acount Status:</span> &nbsp;<?php echo $Allow?> </p>
<div id="error" class="alert"></div>
<div class="Button1 button-one1" id="btn2">Report this account</div>
...
<script>
   $( document ).ready(function() {
    ...
    $('#btn2').click(function(){
      ...
      // alert(email);
      $.ajax({
            url:"reporttSeller.php",
            type:"POST",
            data:{ss:ss,type:type,email:email},
            success: function (data) {
              if(data==1){
              
                $("#error").html("Report is successfully sent");
                $("#error").addClass('alert-success');
                // $('.profile').load(document.URL +  '.profile');
              }
              else if(data==2){
                $("#error").html("Report has already been sent");
