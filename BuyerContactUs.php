...
">
    ...
    <form id="contact-form" name="contact-form" action="BuyerContactUs.php" method="POST">
...
if(isset($_POST['btn'])){
  ...
  $result=mysqli_query($conn,$sql);
  if(mysqli_num_rows($result)>0){
  $sql1="UPDATE feedback SET Feedback='$m',User_Type='Buyer',Date='$date' WHERE Email='$email'";
  if(mysqli_query($conn,$sql1)){
    echo "<div  class='alert alert-success' role='alert'>Thank You for your rating!!
    </div>";
  }
    ...
    $sql1="INSERT INTO feedback (Email,Feedback,User_Type,Date) VALUES('$email','$m','Buyer','$date')";
    if(mysqli_query($conn,$sql1)){
      echo "<div  class='alert alert-success' role='alert'>Thank You for your rating!!
      </div>";
    }
